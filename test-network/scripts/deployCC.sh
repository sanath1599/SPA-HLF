...
if [ "$CC_SRC_PATH" = "NA" ]; then
  ...
  if [ "$CC_NAME" = "spa" ]; then
    println $'\e[0;32m'spa-chaincode$'\e[0m' chaincode
    CC_SRC_PATH="../spa-chaincode"
  elif [ "$CC_NAME" = "spa-richquery" ]; then
    println $'\e[0;32m'spa-chaincode-couchdb$'\e[0m' chaincode
    CC_SRC_PATH="../spa-chaincode-couchdb"
  elif [ "$CC_NAME" = "spa-pdc" ]; then
    println $'\e[0;32m'spa-chaincode-pdc$'\e[0m' chaincode
    CC_SRC_PATH="../spa-chaincode-pdc"
  else
    fatalln "The chaincode name ${CC_NAME} is not supported by this script. Supported chaincode names are: spa, spa-richquery, spa-pdc "
  fi
